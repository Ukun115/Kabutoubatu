...
void CharacterController::Init(const float radius, const float height, const Vector3& position)
{
	m_position = position;
	//コリジョン作成。
	m_radius = radius;
...
const Vector3& CharacterController::Execute(Vector3& moveSpeed, const float deltaTime,bool& isHitGround, Vector3& hitGroundNormal )
{
	isHitGround = false;
	if (moveSpeed.y > 0.0f) {
		//吹っ飛び中にする。
	...
	//XZ平面での衝突検出と衝突解決を行う。
	{
		...
		//当たったかどうかをゲット用の変数に入れる
		m_isHitWall = callback.isHit;
