		...
		void LoadBalancingTransport::sendFrameRaiseEvents(void)
		{
			...
			while(!empty) // Dequeue and process while the queue is not empty
			{
				...
				if(!empty)
				{
					...
					if(mDirect)
						mLBClient.sendDirect(content, 3, SendDirectOptions().setFallbackRelay(true).setReceiverGroup(p.mDebugEchoMode?Lite::ReceiverGroup::ALL:Lite::ReceiverGroup::OTHERS));
					else
					{
						//Encrypt = localVoice.Encrypt
						mLBClient.opRaiseEvent(p.mReliable, content, 3, EventCode::VOICE_DATA, RaiseEventOptions().setChannelID(p.mChannelId).setReceiverGroup(p.mDebugEchoMode?Lite::ReceiverGroup::ALL:Lite::ReceiverGroup::OTHERS).setInterestGroup(p.mInterestGroup));
