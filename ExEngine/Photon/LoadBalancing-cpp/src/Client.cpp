...
#	pragma GCC diagnostic pop
					...
					else
					{
						...
						if(mState == PeerStates::ConnectedToNameserver)
						{
							if(operationResponse.getParameters().contains(ParameterCode::ENCRYPTION_DATA))
							{
								...
								break;
								case EncryptionMode::UDP_TRAFFIC_ENCRYPTION_RANDOM_SEQUENCE_CBC:
								{
		...
		{
			if(mUseAuthOnce)
				return mpPeer->opAuthenticateOnce(mAppID, mAppVersion, M_CONNECTION_PROTOCOL, mUseUDPEncryption?EncryptionMode::UDP_TRAFFIC_ENCRYPTION_RANDOM_SEQUENCE_DEFAULT:EncryptionMode::USER_DATA_ENCRYPTION_NO_DIFFIE_HELLMAN, mAuthenticationValues, mAutoLobbyStats, mSelectedRegion);
			else
