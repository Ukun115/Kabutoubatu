...
)
{
	...
	m_vsNonSkinModel = ResourceBankManager::GetInstance()->GetShaderFromBank(filePath, vsEntryPointFunc);
	if (m_vsNonSkinModel == nullptr) {
		m_vsNonSkinModel = new Shader;
		m_vsNonSkinModel->LoadVS(fxFilePath, vsEntryPointFunc);
		ResourceBankManager::GetInstance()->RegistShaderToBank(filePath, vsEntryPointFunc, m_vsNonSkinModel);
	}
	//スキンありモデル用のシェーダーをロードする。
	m_vsSkinModel = ResourceBankManager::GetInstance()->GetShaderFromBank(filePath, vsSkinEntriyPointFunc);
	if (m_vsSkinModel == nullptr) {
		m_vsSkinModel = new Shader;
		m_vsSkinModel->LoadVS(fxFilePath, vsSkinEntriyPointFunc);
		ResourceBankManager::GetInstance()->RegistShaderToBank(filePath, vsSkinEntriyPointFunc, m_vsSkinModel);
	}

	m_psModel = ResourceBankManager::GetInstance()->GetShaderFromBank(filePath, psEntryPointFunc);
	if (m_psModel == nullptr) {
		m_psModel = new Shader;
		m_psModel->LoadPS(fxFilePath, psEntryPointFunc);
		ResourceBankManager::GetInstance()->RegistShaderToBank(filePath, psEntryPointFunc, m_psModel);
	}
